...
`go-ruby-pg/pg` reimplements Ruby's `pg` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-pg/pg` directly, with no Ruby runtime;
